...
async function cargarHistorialLaboratorio() {
    ...
    try {
        ...
        // Consultamos directamente estudios_laboratorio
      const { data: estudios, error } = await fisioNet
    .from('estudios_laboratorio')
    .select('*') // ¡Solo trae todo de esta tabla!
    ...
    .limit(20);
        ...
        // Guardamos en un caché local simple
        historialLab = estudios; 

        // Renderizamos con los datos del paciente unidos
        contenedor.innerHTML = estudios.map(est => {
           const nom = `${est.paciente_nombre || ''} ${est.apellido_paterno || ''}`.trim();
            return `
    <div class="list-group-item list-group-item-action border-0 border-bottom p-3" 
         style="cursor: pointer;" 
         onclick="abrirDictaminacion('${est.id}')">
        <div class="d-flex justify-content-between align-items-center">
            <span class="fw-bold small text-dark">${nom.toUpperCase()}</span>
            <span class="badge bg-info text-white" style="font-size: 0.6rem;">${est.estado}</span>
        </div>
        <div class="text-muted small mt-1">
            <i class="fas fa-microscope me-1"></i> ${est.estudios_etiquetas || 'General'}
        </div>
...
async function procesarGuardado(idEstudio) {
    ...
    try {
        ...
        document.querySelectorAll('.resultado-input').forEach(input => {
            if (input.value.trim() !== "") {
                // Buscamos el nombre del estudio padre usando el atributo data en el contenedor padre
      ...
      const pdfsExistentes = estudioActual.archivo_pdf_url ? estudioActual.archivo_pdf_url.split(',') : [];
        ...
        if (typeof archivosLaboratorio !== 'undefined' && archivosLaboratorio.length > 0) {
            // Usamos 'id_socio_emisor' o el que tengas para la clínica
            const idClinica = localStorage.getItem('id_clinica_activa'); 
        ...
        // 6. INSERT EN DETALLE (Usamos el array 'resultadosNuevos')
    ...
    let html = `
        <div class="mb-3">
            <div class="alert alert-secondary p-2 small">
                <i class="fas fa-calendar-alt"></i> <strong>Fecha de dictamen:</strong> ${fechaFormateada}
            </div>
        </div>
    `;

    // 4. LÓGICA DIFERENCIADA
    if (estudio.metodo_carga === 'PDF_EXTERNO') {
        const urls = estudio.archivo_pdf_url ? estudio.archivo_pdf_url.split(',') : [];
        
        // Ponemos un mensaje de carga inicial en el contenedor
        contenedor.innerHTML = html + `<div class="text-center p-3"><i class="fas fa-spinner fa-spin me-2"></i>Generando accesos seguros...</div>`;

        let bloquesPdfHtml = `<div class="alert alert-info py-1 px-2 small mb-2">Documento Externo Seguro:</div>`;

        // Iteramos de forma asíncrona para firmar cada archivo guardado
        for (const nombreArchivo of urls) {
            const rutaLimpia = nombreArchivo.trim();
            if (!rutaLimpia) continue;

            try {
                // Generamos la URL firmada dinámicamente desde tu bucket privado
                const { data, error } = await fisioNet.storage
                    .from('expedientes-clinicos')
                    .createSignedUrl(rutaLimpia, 3600); // 1 hora de validez
        ...
        // Inyectamos todo el bloque de PDFs firmados con éxito
        contenedor.innerHTML = html + bloquesPdfHtml;
        return;

    } else {
        // CAPTURA DE QUÍMICO (Se queda exactamente igual, pero optimizado el flujo)
        try {
...
async function subirArchivosASupabase(pacienteId, clinicaId, estudioId) {
    ...
    // Asegúrate de tener una variable global en portal-laboratorio llamada 'archivosLaboratorio'
    // (o cámbiala por la variable donde guardas los archivos seleccionados)
    const nombresArchivos = [];
    ...
    for (const archivo of archivosLaboratorio) {
        ...
        console.log("📤 Subiendo a Supabase:", rutaSegura);
